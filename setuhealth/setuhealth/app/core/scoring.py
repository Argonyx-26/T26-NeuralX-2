...
def is_negated_in_text(kw: str, text: str) -> bool:
    """Checks if all occurrences of a keyword in text are preceded by clinical negation words."""
    negation_prefixes = ["no ", "not ", "none", "denies ", "denied ", "without ", "negative for ", "no known ", "free of "]
    pos = 0
    kw_lower = kw.lower()
    ...
    while True:
        ...
        if idx == -1:
            break
        found_any = True
        window = text_lower[max(0, idx - 25):idx]
        if not any(neg in window for neg in negation_prefixes):
            return False  # Found an affirmative (non-negated) occurrence!
        pos = idx + len(kw_lower)
    return found_any  # If found but all were negated, return True (is negated)
...
def score_symptoms_and_red_flags(symptoms: List[Any]) -> Tuple[float, List[ContributingFactor], bool, List[str]]:
    ...
    for rf in RED_FLAG_PATTERNS:
        for kw in rf["keywords"]:
            if kw.lower() in full_symptom_blob and not is_negated_in_text(kw, full_symptom_blob) and rf["id"] not in matched_red_flags:
    ...
    for mod in MODERATE_SYMPTOM_PATTERNS:
        for kw in mod["keywords"]:
            if kw.lower() in full_symptom_blob and not is_negated_in_text(kw, full_symptom_blob):
